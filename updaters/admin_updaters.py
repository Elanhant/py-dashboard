...
from processors.admin_processors import AdminCustomersByPurchases, AdminRePurchases, AdminRePurchasesTotals, \
	AdminSalesEightWeeks
	...
	def calculate_day_data(self, today_ts, yesterday_ts):
		...
		for k in self.aggregated_data['numerator']:
			try:
				...
				pass
		self.update_aggregated_data(self.aggregated_data['numerator'],
		                            customers_count + self.aggregated_data['denominator'])
...
class CustomersDataUpdater(Updater):
	...
	def calculate_day_data(self, today_ts, yesterday_ts):
		...
		self.cur.execute(
			""" SELECT (SELECT COUNT('id') FROM card WHERE date_create >= %s) + (SELECT COUNT('id') FROM online_data) """,
			(yesterday_ts, ))
		denominator = self.cur.fetchone()[0]
		for k in self.aggregated_data['numerator']:
			try:
...
class ParticipantsCountUpdater(Updater):
	...
	def calculate_day_data(self, today_ts, yesterday_ts):
		self.cur.execute(""" SELECT COUNT(id) FROM card WHERE date_create >= %s AND date_create < %s """,
